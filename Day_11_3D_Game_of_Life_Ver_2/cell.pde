class Cell
{
  ...
  int px = 0;
  int py = 0; //positions in data structure. It's like a setup. 
  int pz = 0;
  boolean isLive = false; //boolean can determine something as true or false
  boolean nextLive = false;
  
  Cell(int _px, int _py, int _pz, float _d) //constructor
  {
    this.px = _px; 
    this.py = _py;
    this.pz = _pz;
  ...
  void display()
  {
    if(isLive)
    {
     fill(0,200,0);
        noStroke();
    pushMatrix(); //sets aside a seperate area to perform a function
    translate((-maxX/2)*this.d, (-maxY/2)*this.d, (-maxZ/2)*this.d);
      translate(this.px * this.d, this.py * this.d, this.pz * this.d);
      box(this.d); //no position needed. 
    popMatrix(); //Like drawing one a piece of paper and then moving the paper around.
    }
    }
